...
% foot length
L_F = 0.2 * scalorFactor; %[m]

% distance CS1 to CG (local center of gravity)
D1G_F = 0.14 * scalorFactor; %[m]

% distance CS1 to C2 (ankle joint)
D12_F = 0.16 * scalorFactor; %[m]16

% distance CS1 to CS3 (heel pad impact point)
D13_F = 0.20 * scalorFactor; %[m]20

% foot mass
m_F  = 1.25 * scalorFactor; %[kg] 1.25
...
% distance CS1 to CG (local center of gravity)
D1G_S = 0.3 * scalorFactor; %[m]

% distance CS1 to C2 (knee joint)
D12_S = 0.5* scalorFactor; %[m]

% shank mass
m_S  = 3.5* scalorFactor; %[kg]
...
% distance CS1 to CG (local center of gravity)
D1G_T = 0.3* scalorFactor; %[m] 

% distance CS1 to C2 (hip joint)
D12_T = 0.5* scalorFactor;    %[m]
D12z_T = 0.1* scalorFactor;   %[m]

% thigh mass
m_T  = 8.5* scalorFactor; %[kg]
...
% HAT length
L_HAT = 0.8* scalorFactor; %[m]

% distance hip to center of gravity
D1G_HAT = 0.35* scalorFactor; %[m]

% HAT mass
m_HAT = 53.5* scalorFactor; %[kg]
...
massHAB   = 1.145; % muscle mass
t1fHAB    = 0.5;  % type 1 muscle fibers
...
massHAD   = 1.91; % muscle mass
t1fHAD    = 0.5;  % type 1 muscle fibers
...
massHFL   = 1.87; % muscle mass
t1fHFL    = 0.5;  % type 1 muscle fibers
...
massGLU   = 1.4; % muscle mass
t1fGLU    = 0.5;  % type 1 muscle fibers
...
massHAM   = 1.82; % muscle mass
t1fHAM    = 0.44;  % type 1 muscle fibers
...
massRF   = 1; % muscle mass [UNKNOWN]
t1fRF    = 0.5;  % type 1 muscle fibers [UNKNWON]
...
massVAS   = 2.91; % muscle mass
t1fVAS    = 0.5;  % type 1 muscle fibers
...
massBFSH   = 2; % muscle mass [UNKNWON]
t1fBFSH    = 0.5;  % type 1 muscle fibers [UNKNWON]
...
massGAS   = 0.45; % muscle mass
t1fGAS    = 0.54;  % type 1 muscle fibers
...
massSOL   = 1.36; % muscle mass
t1fSOL    = 0.81;  % type 1 muscle fibers
...
massTA   = 0.29; % muscle mass
t1fTA    = 0.7;  % type 1 muscle fibers
...
%% Total mass
m_Total = m_F*2+m_S*2+m_T*2+m_HAT; 

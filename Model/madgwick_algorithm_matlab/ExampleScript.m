...
for t = 1:length(time)
    ...
    quaternion_AGM(t, :) = AHRS.Quaternion; 
    
    %use UpdateIMU if you have no magnetometer data
    AHRS.UpdateIMU(Gyroscope(t,:) * (pi/180), Accelerometer(t,:)); 
    quaternion_AG(t, :) = AHRS.Quaternion;
end
...
euler_AGM = quatern2euler(quaternConj(quaternion_AGM)) * (180/pi);	% use conjugate for sensor frame relative to Earth and convert to degrees.
...
hold on;
plot(time, euler_AG(:,1), 'r');
plot(time, euler_AG(:,2), 'g');
plot(time, euler_AG(:,3), 'b');

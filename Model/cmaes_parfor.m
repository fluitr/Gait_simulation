...
defopts.ParforRun = 'no % Parfor Run is able to do Parallel simulation ';
defopts.ParforWorkers = '2 % Parfor Amount of workers';
...
flgParforRun = myevalbool(opts.ParforRun);
flgParforWorkers = myevalbool(opts.ParforWorkers);
...
if flgresume % resume is on
  ...
  if myevalbool(opts.EvalInitialX)
    ...
    %% Display
    load('OptData/OptSaver')
    val_list = [val_list; fitness.hist(1) Tend];
    save('OptData/OptSaver.mat','val_list','i_opts', 'xmean'); clear val_list i_opts
...
while isempty(stopflag)
  ...
  % Setting outcome_parfor to nan values
  for i_op = 1:(lambda)
    outcome_parfor{i_op} = nan;
  end
  
  while ~isempty(find(isnan(cell2mat(outcome_parfor))))
  parfor (k=find(isnan(cell2mat(outcome_parfor))),flgParforWorkers) %% PARFOR

    % Resample, until fitness is not NaN
	% while isnan(outcome_parfor{k})
        %arz(:,k) = randn(N,1); % (re)sample
        par_arz{k} = randn(N,1); % (re)sample
        
        if flgDiagonalOnly
            %arx(:,k) = xmean + sigma * diagD .* arz(:,k);              % Eq. (1)
            par_arx{k} = xmean + sigma * diagD .* par_arz{k};              % Eq. (1)
        else
            %arx(:,k) = xmean + sigma * (BD * arz(:,k));                % Eq. (1)
            par_arx{k} = xmean + sigma * (BD * par_arz{k});                % Eq. (1)
        end
      ...
      if ~bnd.isactive
        %arxvalid(:,k) = arx(:,k);
        par_arxvalid{k} = par_arx{k};
      else
        %arxvalid(:,k) = xintobounds(arx(:,k), lbounds, ubounds);
        par_arxvalid{k} = xintobounds(par_arx{k}, lbounds, ubounds);
      end
    ...
    if ~isempty(find(isnan(cell2mat(outcome_parfor))))
        ...
        % Out of cell        
        outcome_parfor = cell2mat(outcome_parfor);

        % Sort
        [outcome_parfor,I] = sort(outcome_parfor);
        
        % Sort rest
        par_arz = par_arz(I);
        par_arx = par_arx(I);
        par_arxvalid = par_arxvalid(I);
        
        %Revert outcome_parfor
        outcome_parfor = num2cell(outcome_parfor);     
    end
    %Counting nan-values
  countevalNaN = countevalNaN + sum(isnan(cell2mat(outcome_parfor)'));  
  
  end
  fitness.raw(1:lambda) = cell2mat(outcome_parfor)'; 
  counteval = counteval + sum(~isnan(fitness.raw(1:lambda))); % retries due to NaN are not counted
  
  arz = cell2mat(par_arz);
  arx = cell2mat(par_arx);
  arxvalid = cell2mat(par_arxvalid);
  end
  ...
  for k=find(isnan(fitness.raw)), 
    % fitness.raw(k) = NaN; 
    tries = flgParforRun;  % in parallel case this is the first re-trial
    % Resample, until fitness is not NaN
  ...
  %% Display
  load('OptData/OptSaver')
  val_list = [val_list; fitness.sel' ones(length(fitness.sel),1)*Tend/length(fitness.sel)];
  TrialNum = counteval;
  MinOveral = min(val_list(:,1));
  MeanValLast = mean(val_list(end-lambda+1:end,1));
  MinValLast = min(val_list(end-lambda+1:end,1));
  TRunLast = Tend/length(fitness.sel);
